...
def parse_hex_color(value: str) -> tuple[int, int, int]:
    ...
    return tuple(int(value[index : index + 2], 16) for index in (1, 3, 5))  # ty: ignore[invalid-return-type]
...
def remove_chroma_background(
    ...
    pixels = rgba.load()
    if pixels is None:
        raise ValueError("failed to load RGBA pixels from image")
    for y in range(rgba.height):
        for x in range(rgba.width):
            red, green, blue, _ = pixels[x, y]  # ty: ignore[not-iterable]
