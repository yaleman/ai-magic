...
def render(rows: list[Row], include_delivery: bool) -> str:
    ...
    for i, row in enumerate(rows, 1):
        ...
        if include_delivery:
            delivery = (
                "unknown" if row.delivery_aud is None else money(row.delivery_aud)
            )
            lines.append(
...
def main() -> int:
    ...
    for i, line in enumerate(raw):
        if not isinstance(line, dict):
            ...
            continue
        try:
            rows.append(parse_row(line, args.include_delivery))
    ...
    rows.sort(
        key=lambda r: (
            r.effective_per_litre,
            r.effective_price_aud,
            r.retailer.casefold(),
            r.product.casefold(),
        )
    )
